...
#define AVX512_SHUF_MASK(a, b, c, d) ((a) | ((b) << 2) | ((c) << 4) | ((d) << 6))
...
static void transpose_and_interleave_64words_avx512(__m512i matrix[8]) {
     ...
     *    we can use them instead of permutation operations in the first two levels.
     *    This does not directly result in a speed-up (all operations have roughly the
     *    same performance characteristics according to the Intel Intrinsic Guide).
     *    The big advantage is that the `_mm512_shuffle_i32x4` takes the permutation
     *    as an immediate compile time constant: we do not need an extra register to
     *    prescribe the shuffle.
     *
     ...
     *
     *    The down-side is that these operations cannot freely pick between the two
     *    source operands (the first 256bit come from the first, the rest from the second).
     *    Hence, the shuffle in Level2 is not exactly as one would expect from Hacker's Delight
     *    (the second and third 128 bit lanes are switched), but Level2 fix the issues
     *    (again resulting in a somewhat unexpected permutation sequence).
     *
     *  - The function is forced to be inlined; thus there are no additional load/stores
     *    to satisfy the ABI (checked for GCC11).
     */

    // LEVEL0
    {
        const int top = AVX512_SHUF_MASK(0, 1, 0, 1);
        const int bottom = AVX512_SHUF_MASK(2, 3, 2, 3);

        AVX512_SWAP_IMM(0, 4, top, bottom);
        AVX512_SWAP_IMM(1, 5, top, bottom);
        AVX512_SWAP_IMM(2, 6, top, bottom);
        AVX512_SWAP_IMM(3, 7, top, bottom);
    }

    // LEVEL1
    {
        const int top = AVX512_SHUF_MASK(0, 2, 0, 2);
        const int bottom = AVX512_SHUF_MASK(1, 3, 1, 3);

        AVX512_SWAP_IMM(0, 2, top, bottom);
        AVX512_SWAP_IMM(1, 3, top, bottom);
        AVX512_SWAP_IMM(4, 6, top, bottom);
        AVX512_SWAP_IMM(5, 7, top, bottom);
    }
    ...
    // LEVEL2
    {
        ...
        const __m512i bottom = _mm512_setr_epi32(0x00 + 2, 0x10 + 2, //
                                                 0x08 + 2, 0x18 + 2, //
                                                 0x04 + 2, 0x14 + 2, //
                                                 0x0c + 2, 0x1c + 2, //
        //
        ...
        AVX512_SWAP32(0, 1, top, bottom);
        AVX512_SWAP32(2, 3, top, bottom);
        AVX512_SWAP32(4, 5, top, bottom);
        AVX512_SWAP32(6, 7, top, bottom);
    }
